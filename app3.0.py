...
pestaña_codigos = hoja_principal.worksheet("Códigos") # NUEVA PESTAÑA AGREGADA

# --- 2. SEGURIDAD, AUTENTICACIÓN Y CÓDIGOS ---
def verificar_y_quemar_codigo(codigo_ingresado):
    """
    Verifica que el código venga de una compra válida (Make lo dejó en 'Usado')
    y lo quema pasándolo a 'Activado' para que no se pueda reutilizar.
    """
    try:
        ...
        if celda_codigo:
            ...
            if estado == "Usado":
                # Cambiamos a Activado para bloquearlo de futuros registros
...
def register_user(username, password, nombre, sexo, estatura, edad):
    # La validación de existencia de usuario la movemos a la UI para no quemar el código en vano
    hashed_pw = hash_password(password)
...
if not st.session_state['logged_in']:
    ...
    if choice == "Iniciar Sesión":
        ...
        # NUEVO CAMPO OBLIGATORIO PARA EL CÓDIGO
        ...
        if st.button("Crear Cuenta"):
            if not consentimiento:
                ...
                # 1. Validar que el nombre de usuario no esté tomado ANTES de quemar el código
                ...
                if new_username in usuarios_existentes:
                    ...
                    # 2. Verificar y quemar el código
                    es_valido, msj_codigo = verificar_y_quemar_codigo(new_codigo)
                    
                    if es_valido:
                        # 3. Registrar al usuario si el código era válido
